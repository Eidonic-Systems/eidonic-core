...
function Wait-For-Health {
    ...
    throw "Temporary routing-candidate Orchestrator did not become reachable at $BaseUrl/health."
}
...
if ([string]::IsNullOrWhiteSpace($configuredControlModel)) {
    throw "EIDON_PROVIDER_MODEL must be set in .env for routing candidate validation."
}

if ([string]::IsNullOrWhiteSpace($configuredCandidateModel)) {
    throw "EIDON_DOMAIN_ROUTE_CANDIDATE_MODEL must be set in .env for routing candidate validation."
}

if ($configuredCandidateModel -eq $configuredControlModel) {
    throw "EIDON_DOMAIN_ROUTE_CANDIDATE_MODEL must differ from EIDON_PROVIDER_MODEL for routing candidate validation."
}

if ($configuredBackend -ne 'ollama') {
    throw "Routing candidate validation requires EIDON_PROVIDER_BACKEND=ollama."
}
...
Write-Host ""
Write-Host "Routing candidate success provenance invariant validation:" -ForegroundColor Yellow
...
try {
    ...
    Write-Host ""
    Write-Host "Temporary routing-candidate baseline health payload:" -ForegroundColor Yellow
    Write-Host ($baselineHealth | ConvertTo-Json -Depth 12)

    $warmResponse = Invoke-RestMethod -Uri "$baseUrl/provider/warm" -Method Post

    Write-Host ""
    Write-Host "Routing-candidate warm payload:" -ForegroundColor Yellow
    Write-Host ($warmResponse | ConvertTo-Json -Depth 12)

    $postWarmHealth = Invoke-RestMethod -Uri "$baseUrl/health" -Method Get

    Write-Host ""
    Write-Host "Routing-candidate post-warm health payload:" -ForegroundColor Yellow
    Write-Host ($postWarmHealth | ConvertTo-Json -Depth 12)

    $stamp = Get-Date -Format 'yyyyMMddHHmmssfff'
    $sessionId = "session-routing-candidate-$stamp"
    $signalId = "sig-routing-candidate-$stamp"
    ...
    Write-Host ""
    Write-Host "Routing-candidate orchestration payload:" -ForegroundColor Yellow
    ...
    Write-Host ""
    Write-Host "Routing-candidate orchestration response:" -ForegroundColor Yellow
    Write-Host ($orchestrateResponse | ConvertTo-Json -Depth 12)

    $artifactId = [string]$orchestrateResponse.artifact_id
    if ([string]::IsNullOrWhiteSpace($artifactId)) {
        throw "Routing-candidate orchestration response did not include an artifact_id."
    }

    $lineageId = [string]$orchestrateResponse.lineage_id
    if ([string]::IsNullOrWhiteSpace($lineageId)) {
        throw "Routing-candidate orchestration response did not include a lineage_id."
    }
    ...
    Write-Host ""
    Write-Host "Routing-candidate artifact retrieval response:" -ForegroundColor Yellow
    Write-Host ($artifactResponse | ConvertTo-Json -Depth 12)

    Write-Host ""
    Write-Host "Routing-candidate lineage retrieval response:" -ForegroundColor Yellow
    ...
    if ($null -ne $artifact -and $null -ne $lineage) {
        ...
        Assert-EqualValue -Label 'artifact.provider_model' -Expected $configuredCandidateModel -Actual $artifact.provider_model -Failures $failures
        Assert-EqualValue -Label 'artifact.provider_status' -Expected 'succeeded' -Actual $artifact.provider_status -Failures $failures
        Assert-EqualValue -Label 'artifact.provider_route_mode' -Expected 'candidate' -Actual $artifact.provider_route_mode -Failures $failures
        Assert-EqualValue -Label 'artifact.provider_route_reason' -Expected 'candidate_domain_route' -Actual $artifact.provider_route_reason -Failures $failures
    ...
    Write-Host ""
    Write-Host "Routing candidate success provenance invariant validation summary:" -ForegroundColor Yellow
    Write-Host ($summary | ConvertTo-Json -Depth 8)

    if ($summary.status -ne 'passed') {
        throw "Routing candidate success provenance invariant validation failed."
    }

    Write-Host ""
    Write-Host "Routing candidate success provenance invariant validation passed." -ForegroundColor Green
}
finally {
    if ($null -ne $process) {
