...
function Wait-For-Health {
    ...
    throw "Temporary routing-fallback Orchestrator did not become reachable at $BaseUrl/health."
}
...
if ([string]::IsNullOrWhiteSpace($configuredControlModel)) {
    throw "EIDON_PROVIDER_MODEL must be set in .env for routing fallback validation."
}

if ($configuredBackend -ne 'ollama') {
    throw "Routing fallback validation requires EIDON_PROVIDER_BACKEND=ollama."
}

$stamp = Get-Date -Format 'yyyyMMddHHmmssfff'
$missingCandidateModel = "phase2-missing-route-candidate-$stamp"

Write-Host ""
Write-Host "Routing fallback provenance invariant validation:" -ForegroundColor Yellow
...
Write-Host ("  forced missing candidate model: {0}" -f $missingCandidateModel)

$overrides = [ordered]@{
    EIDON_PROVIDER_BACKEND = $configuredBackend
    EIDON_PROVIDER_MODEL = $configuredControlModel
    EIDON_DOMAIN_ROUTING_ENABLED = 'true'
    EIDON_DOMAIN_ROUTE_CANDIDATE_MODEL = $missingCandidateModel
}
...
try {
    ...
    Write-Host ""
    Write-Host "Temporary routing-fallback health payload:" -ForegroundColor Yellow
    Write-Host ($baselineHealth | ConvertTo-Json -Depth 12)

    $warmResponse = Invoke-RestMethod -Uri "$baseUrl/provider/warm" -Method Post

    Write-Host ""
    Write-Host "Routing-fallback warm payload:" -ForegroundColor Yellow
    Write-Host ($warmResponse | ConvertTo-Json -Depth 12)

    $sessionId = "session-routing-fallback-$stamp"
    $signalId = "sig-routing-fallback-$stamp"
    ...
    $payload = [ordered]@{
        ...
        content = [ordered]@{
            text = 'Summarize artifact lineage discipline in one sentence.'
        }
    }

    Write-Host ""
    Write-Host "Routing-fallback orchestration payload:" -ForegroundColor Yellow
    ...
    Write-Host ""
    Write-Host "Routing-fallback orchestration response:" -ForegroundColor Yellow
    Write-Host ($orchestrateResponse | ConvertTo-Json -Depth 12)

    $artifactId = [string]$orchestrateResponse.artifact_id
    if ([string]::IsNullOrWhiteSpace($artifactId)) {
        throw "Routing-fallback orchestration response did not include an artifact_id."
    }

    $lineageId = [string]$orchestrateResponse.lineage_id
    if ([string]::IsNullOrWhiteSpace($lineageId)) {
        throw "Routing-fallback orchestration response did not include a lineage_id."
    }
    ...
    Write-Host ""
    Write-Host "Routing-fallback artifact retrieval response:" -ForegroundColor Yellow
    Write-Host ($artifactResponse | ConvertTo-Json -Depth 12)

    Write-Host ""
    Write-Host "Routing-fallback lineage retrieval response:" -ForegroundColor Yellow
    ...
    if ([string](Get-OptionalProperty -Object $baselineHealth.provider -Name 'route_candidate_status') -ne 'provider_model_missing') {
        Add-Failure -Failures $failures -Message "baseline provider.route_candidate_status was not provider_model_missing"
    }
    ...
    if ($null -eq $warmResponse.provider) {
        ...
        Assert-EqualValue -Label 'warm provider.route_candidate_model' -Expected $missingCandidateModel -Actual (Get-OptionalProperty -Object $warmResponse.provider -Name 'route_candidate_model') -Failures $failures
        Assert-EqualValue -Label 'warm provider.route_candidate_status' -Expected 'provider_model_missing' -Actual (Get-OptionalProperty -Object $warmResponse.provider -Name 'route_candidate_status') -Failures $failures
    }
    ...
    if ($null -ne $artifact -and $null -ne $lineage) {
        ...
        Assert-EqualValue -Label 'artifact.provider_route_mode' -Expected 'fallback' -Actual $artifact.provider_route_mode -Failures $failures
        Assert-EqualValue -Label 'artifact.provider_route_reason' -Expected 'control_fallback_after_candidate_failure' -Actual $artifact.provider_route_reason -Failures $failures
        ...
        Assert-EqualValue -Label 'artifact.governance_outcome' -Expected 'allow' -Actual $artifact.governance_outcome -Failures $failures
        Assert-EqualValue -Label 'artifact.governance_reason' -Expected 'normal_orchestration_path' -Actual $artifact.governance_reason -Failures $failures
        Assert-EqualValue -Label 'artifact.governance_rule_id' -Expected 'default_success_allow' -Actual $artifact.governance_rule_id -Failures $failures

        if ([string]::IsNullOrWhiteSpace([string]$artifact.governance_manifest_version)) {
            Add-Failure -Failures $failures -Message 'artifact.governance_manifest_version was empty on routing fallback success path'
        }
    }
    ...
    $summary = [ordered]@{
        ...
        forced_missing_candidate_model = $missingCandidateModel
    ...
    Write-Host ""
    Write-Host "Routing fallback provenance invariant validation summary:" -ForegroundColor Yellow
    Write-Host ($summary | ConvertTo-Json -Depth 8)

    if ($summary.status -ne 'passed') {
        throw "Routing fallback provenance invariant validation failed."
    }

    Write-Host ""
    Write-Host "Routing fallback provenance invariant validation passed." -ForegroundColor Green
}
finally {
    if ($null -ne $process) {
