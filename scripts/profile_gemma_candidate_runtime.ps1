param(
    ...
    [int]$ControlPort = 8024,
    [int]$CandidatePort = 8025,
...
"@
    ...
    try {
        Write-Host ""
        Write-Host "Starting isolated runtime profile for model: $ModelName" -ForegroundColor Yellow
        ...
        $evalOutputPath = Join-Path $ProfilesDir ("runtime_profile_eval_{0}.json" -f (Sanitize-Label -Value $ModelName))
        ...
        & powershell -ExecutionPolicy Bypass -File $EvalScript `
            ...
            -OutputPath $evalOutputPath

        if ($LASTEXITCODE -ne 0) {
            throw "Eval run failed for model '$ModelName'."
        }
...
if ([string]::IsNullOrWhiteSpace($OutputPath)) {
    ...
    $OutputPath = Join-Path $ProfilesDir ("gemma_runtime_profile_{0}.json" -f $timestamp)
}

$control = Run-IsolatedProfile `
...
if ($summary.status -ne "passed") {
    throw "Gemma candidate runtime profile surface failed."
}

Write-Host ""
Write-Host "Gemma candidate runtime profile surface passed." -ForegroundColor Green
