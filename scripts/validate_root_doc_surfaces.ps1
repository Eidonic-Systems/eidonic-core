...
$rootDocChecks = @($manifest.root_doc_surface_checks)

if ($rootDocChecks.Count -eq 0) {
    throw "Recovery-surface manifest has no root_doc_surface_checks."
}

$failures = [System.Collections.ArrayList]::new()
$checkedSurfaces = [System.Collections.ArrayList]::new()

foreach ($surfaceCheck in $rootDocChecks) {
    $relativePath = [string]$surfaceCheck.path
    $requiredPatterns = @($surfaceCheck.required_patterns)

    if ([string]::IsNullOrWhiteSpace($relativePath)) {
        Add-Failure -Failures $failures -Message "root doc surface check missing path"
        continue
    }

    $absolutePath = Join-Path $resolvedRepoRoot $relativePath
    if (-not (Test-Path $absolutePath)) {
        Add-Failure -Failures $failures -Message ("Missing required root doc surface at {0}" -f $absolutePath)
        continue
    }
...
Write-Host ""
Write-Host "Root-doc surface validation summary:" -ForegroundColor Yellow
Write-Host ($summary | ConvertTo-Json -Depth 8)

if ($summary.status -ne "passed") {
    throw "Root-doc surface validation failed."
}

Write-Host ""
Write-Host "Root-doc surface validation passed." -ForegroundColor Green
