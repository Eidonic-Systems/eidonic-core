param(
    ...
    [int]$CandidateProviderTimeoutSeconds = 120,

    [string]$CasesPath = ".\evals\domain_task_eval_cases.json",

    [string]$BaselinePath = ".\evals\baselines\domain_task_eval_baseline.json"
)
...
$CompareScript = Join-Path $RepoRoot "scripts\compare_domain_task_eval_to_baseline.ps1"
...
$currentResultsPath = Join-Path $CandidateDir ("domain_task_eval_candidate_{0}.json" -f $sanitizedLabel)
...
try {
    Write-Host ""
    Write-Host "Starting isolated domain-task candidate comparison for model: $CandidateModel" -ForegroundColor Yellow
    ...
    if ($null -eq $warmResponse.provider -or $warmResponse.provider.model -ne $CandidateModel) {
        throw "Candidate warmup model mismatch. Expected '$CandidateModel' but got '$($warmResponse.provider.model)'."
    }
    ...
    Write-Host ""
    Write-Host ("Domain-task candidate comparison completed. Results file: {0}" -f $currentResultsPath) -ForegroundColor Green
}
finally {
